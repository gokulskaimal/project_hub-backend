...
export function loadConfig(): AppConfig {
  ...
  const config: AppConfig = {
    ...
    frontend: {
      ...
      resetPasswordPath:
        process.env.RESET_PASSWORD_PATH || "/auth/reset-password",
    ...
    features: {
      emailVerificationRequired: parseBoolean(
        process.env.FEATURE_EMAIL_VERIFICATION_REQUIRED,
        true,
      ),
      invitationRequired: parseBoolean(
        process.env.FEATURE_INVITATION_REQUIRED,
        false,
      ),
      passwordResetEnabled: parseBoolean(
        process.env.FEATURE_PASSWORD_RESET_ENABLED,
        true,
      ),
      multiOrgSupport: parseBoolean(
        process.env.FEATURE_MULTI_ORG_SUPPORT,
        true,
      ),
    },
  };
  ...
  if (config.nodeEnv === "production") {
    ...
    if (usingDefaultSecrets) {
      throw new Error(
        "Default secrets detected in production. Please set proper JWT and cookie secrets.",
      );
    }
  }
...
export function validateConfig(config: AppConfig): void {
  ...
  if (config.session.maxAge < 60000) {
    // Minimum 1 minute
    errors.push(
      "SESSION_MAX_AGE must be at least 60000 milliseconds (1 minute)",
    );
  }
